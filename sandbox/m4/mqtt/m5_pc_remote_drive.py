...
Authors: David Fisher and Squad.
...
def main():
    ...
    forward_button['command'] = lambda: press_forward_button(mqtt_client, left_speed_entry.get(), right_speed_entry.get())
    root.bind('<Up>', lambda event: press_forward_button(mqtt_client, left_speed_entry.get(), right_speed_entry.get()))

    left_button = ttk.Button(main_frame, text="Left")
    left_button.grid(row=3, column=0)
    # left_button and '<Left>' key
    left_button['command'] = lambda: press_left_button(mqtt_client, right_speed_entry.get())
    root.bind('<Left>', lambda event: press_left_button(mqtt_client, right_speed_entry.get()))

    stop_button = ttk.Button(main_frame, text="Stop")
    stop_button.grid(row=3, column=1)
    # stop_button and '<space>' key (note, does not need left_speed_entry, right_speed_entry)
    stop_button['command'] = lambda: press_stop_button(mqtt_client)
    root.bind('<space>', lambda event: press_stop_button(mqtt_client))

    right_button = ttk.Button(main_frame, text="Right")
    right_button.grid(row=3, column=2)
    # right_button and '<Right>' key
    right_button['command'] = lambda: press_right_button(mqtt_client, left_speed_entry.get())
    root.bind('<Right>', lambda event: press_right_button(mqtt_client, left_speed_entry.get()))

    back_button = ttk.Button(main_frame, text="Back")
    back_button.grid(row=4, column=1)
    # back_button and '<Down>' key
    back_button['command'] = lambda: press_back_button(mqtt_client, left_speed_entry.get(), right_speed_entry.get())
    root.bind('<Down>', lambda event: press_back_button(mqtt_client, left_speed_entry.get(), right_speed_entry.get()))
...
def press_left_button(mqtt_client, right_speed):
    print('Left button pressed.')
    mqtt_client.send_message("left_button", [right_speed])
...
def press_right_button(mqtt_client, left_speed):
    print('Right button pressed.')
    mqtt_client.send_message("right_button", [left_speed])
