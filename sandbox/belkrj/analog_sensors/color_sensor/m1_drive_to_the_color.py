...
def drive_to_color(button_state, robot, color_to_seek):
    ...
    if button_state:
        ...
        if robot.color_sensor.color == robot.color_sensor.COLOR_RED:
            robot.left_motor.STOP_ACTION_BRAKE()
            robot.right_motor.STOP_ACTION_BRAKE()

        if robot.color_sensor.color == robot.color_sensor.COLOR_BLUE:
            robot.left_motor.STOP_ACTION_BRAKE()
            robot.right_motor.STOP_ACTION_BRAKE()

        if robot.color_sensor.color == robot.color_sensor.COLOR_BLACK:
            robot.left_motor.STOP_ACTION_BRAKE()
            robot.right_motor.STOP_ACTION_BRAKE()
